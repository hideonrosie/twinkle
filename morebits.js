...
(function (window, document, $) { // Wrap entire file with anonymous function
	...
	Morebits.unbinder = function Unbinder(string) {
		if (typeof string !== 'string') {
			throw new Error('không phải là 1 chuỗi');
		}
		this.content = string;
		this.counter = 0;
		this.history = {};
	...
	Morebits.date.localeData = {
		...
		relativeTimes: {
			thisDay: '[Hôm nay lúc] h:mm A',
			prevDay: '[Ngày hôm qua lúc] h:mm A',
			nextDay: '[Ngày mai lúc] h:mm A',
			thisWeek: 'dddd [lúc] h:mm A',
			pastWeek: '[Lần cuối] dddd [lúc] h:mm A',
			other: 'YYYY-MM-DD'
		}
	};
	...
	Morebits.wiki.api.prototype = {
		...
		returnError: function(callerAjaxParameters) {
			if (this.errorCode === 'badtoken' && !this.badtokenRetry) {
				this.statelem.warn('Mã token không hợp lệ. Nhận mã mới và thử lại ...');
	...
	Morebits.wiki.api.getToken = function() {
		var tokenApi = new Morebits.wiki.api('Đang lấy token', {
			action: 'query',
			meta: 'tokens',
			type: 'csrf'
		});
	...
	Morebits.wiki.page = function(pageName, currentAction) {
		...
		if (!currentAction) {
			currentAction = 'Đang mở trang "' + pageName + '"';
		}
		...
		this.load = function(onSuccess, onFailure) {
			...
			ctx.loadApi = new Morebits.wiki.api('Đang lấy trang...', ctx.loadQuery, fnLoadSuccess, ctx.statusElement, ctx.onLoadFailure);
		...
		this.save = function(onSuccess, onFailure) {
			...
			if (ctx.fullyProtected && !ctx.suppressProtectWarning &&
				!confirm('Bạn chuẩn bị chỉnh sửa trang được bảo vệ hoàn toàn "' + ctx.pageName +
				(ctx.fullyProtected === 'infinity' ? '" (được bảo vệ vô thời hạn)' : '" (bảo vệ hết hạn ' + new Morebits.date(ctx.fullyProtected).calendar('utc') + ' (UTC))') +
				'.  \n\nNhấp vào OK để tiếp tục chỉnh sửa hoặc Cancel để bỏ qua chỉnh sửa này.')) {
				ctx.statusElement.error('Chỉnh sửa trang được bảo vệ hoàn toàn đã bị hủy bỏ.');
				ctx.onSaveFailure(this);
				return;
			}
			...
			ctx.saveApi = new Morebits.wiki.api('Đang lưu trang...', query, fnSaveSuccess, ctx.statusElement, fnSaveError);
		...
		this.lookupCreation = function(onSuccess) {
			...
			ctx.lookupCreationApi = new Morebits.wiki.api('Đang lấy thông tin tạo trang', query, fnLookupCreationSuccess, ctx.statusElement);
		...
		this.move = function(onSuccess, onFailure) {
			...
			if (fnCanUseMwUserToken('move')) {
				...
				ctx.moveApi = new Morebits.wiki.api('đang lấy token...', query, fnProcessMove, ctx.statusElement, ctx.onMoveFailure);
		...
		this.patrol = function() {
			...
			if ($('.patrollink').length) {
				...
				ctx.patrolApi = new Morebits.wiki.api('đang lấy token...', patrolQuery, fnProcessPatrol);
		...
		this.triage = function() {
			...
			if (new mw.Title(Morebits.pageNameNorm).getPrefixedText() === new mw.Title(ctx.pageName).getPrefixedText()) {
				...
				statelem.status('Hàm triage bị lỗi 3...');
		...
		this.deletePage = function(onSuccess, onFailure) {
			...
			if (fnCanUseMwUserToken('delete')) {
				...
				ctx.deleteApi = new Morebits.wiki.api('đang lấy token...', query, fnProcessDelete, ctx.statusElement, ctx.onDeleteFailure);
		...
		this.undeletePage = function(onSuccess, onFailure) {
			...
			if (fnCanUseMwUserToken('undelete')) {
				...
				ctx.undeleteApi = new Morebits.wiki.api('đang lấy token...', query, fnProcessUndelete, ctx.statusElement, ctx.onUndeleteFailure);
		...
		this.protect = function(onSuccess, onFailure) {
			...
			ctx.protectApi = new Morebits.wiki.api('đang lấy token...', query, fnProcessProtect, ctx.statusElement, ctx.onProtectFailure);
		...
		this.stabilize = function(onSuccess, onFailure) {
			...
			// if a non-admin tries to do this, don't bother
			if (!Morebits.userIsSysop) {
				ctx.statusElement.error('Không thể áp dụng cài đặt FlaggedRevs: chỉ quản trị viên mới có thể làm điều đó');
				ctx.onStabilizeFailure(this);
				return;
			}
			if (!ctx.flaggedRevs) {
				ctx.statusElement.error('Lỗi nội bộ: bạn phải đặt flaggedRevs trước khi gọi stable ()!');
				ctx.onStabilizeFailure(this);
				return;
			}
			if (!ctx.editSummary) {
				ctx.statusElement.error('Lỗi nội bộ: lý do không được đặt trước khi gọi stable () (sử dụng hàm setEditSummary)!');
				ctx.onStabilizeFailure(this);
				return;
			}
			...
			if (fnCanUseMwUserToken('stabilize')) {
				...
				ctx.stabilizeApi = new Morebits.wiki.api('đang lấy token...', query, fnProcessStabilize, ctx.statusElement, ctx.onStabilizeFailure);
		...
		var fnLoadSuccess = function() {
			...
			if (!ctx.csrfToken) {
				ctx.statusElement.error('Không truy xuất được mã token chỉnh sửa.');
				ctx.onLoadFailure(this);
				return;
			}
			ctx.loadTime = $(xml).find('api').attr('curtimestamp');
			if (!ctx.loadTime) {
				ctx.statusElement.error('Không truy xuất được nhãn thời gian hiện tại.');
				ctx.onLoadFailure(this);
				return;
			}
			...
			if (ctx.editMode === 'revert') {
				...
				if (!ctx.revertCurID) {
					ctx.statusElement.error('Không truy xuất được ID của bản sửa đổi hiện tại.');
					ctx.onLoadFailure(this);
					return;
				}
				...
				if (!ctx.revertUser) {
					if ($(xml).find('rev').attr('userhidden') === '') {  // username was RevDel'd or oversighted
						...
						ctx.statusElement.error('Không truy xuất được người dùng đã thực hiện sửa đổi.');
						ctx.onLoadFailure(this);
						return;
					}
				}
				// set revert edit summary
				ctx.editSummary = '[[Wikipedia:Hồi sửa]] bản sửa đổi ' + ctx.revertOldID + ' bởi ' + ctx.revertUser + ': ' + ctx.editSummary;
			}
		...
		var fnCheckPageName = function(xml, onFailure) {
			...
			if ($(xml).find('page').attr('invalid') === '') {
				ctx.statusElement.error('Tiêu đề trang không hợp lệ: ' + ctx.pageName);
				onFailure(this);
				return false; // abort
			}
			...
			if ($(xml).find('page').attr('title')) {
				...
				if ($(xml).find('redirects').length > 0) {
					...
					if (origNs !== newNs && !ctx.followCrossNsRedirect) {
						ctx.statusElement.error(ctx.pageName + ' là một chuyển hướng không gian tên chéo đến ' + resolvedName + ', hủy bỏ');
						onFailure(this);
						return false;
					}
	
					// only notify user for redirects, not normalization
					Morebits.status.info('Thông tin', 'Đã đổi hướng từ ' + ctx.pageName + ' đến ' + resolvedName);
				}
				...
				ctx.statusElement.error('Không thể giải quyết chuyển hướng cho: ' + ctx.pageName);
				onFailure(this);
		...
		var fnSaveSuccess = function() {
			...
			if ($(xml).find('edit').attr('result') === 'Success') {
				...
				ctx.statusElement.info(['đã hoàn tất (', link, ')']);
			...
			if ($(xml).find('captcha').length > 0) {
				ctx.statusElement.error('Không thể lưu trang vì máy chủ wiki muốn bạn điền CAPTCHA.');
			} else {
				ctx.statusElement.error('Đã nhận được lỗi không xác định từ API khi lưu trang');
			}
		...
		var fnSaveError = function() {
			...
			if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
				...
				var purgeApi = new Morebits.wiki.api('Edit conflict detected, purging server cache', purgeQuery, function() {
					...
					ctx.statusElement.info('Phát hiện sửa đổi xung đột, áp dụng lại sửa đổi');
					if (fnCanUseMwUserToken('edit')) {
			...
			// check for network or server error
				...
				ctx.statusElement.info('Lưu thất bại, đang thử lại');
				--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
			...
			// hard error, give up
				...
				if (errorCode === 'protectedpage') {
					ctx.statusElement.error('Không lưu được sửa đổi: Trang bị khóa');
				// check for absuefilter hits: disallowed or warning
					...
					if (errorCode === 'abusefilter-disallowed') {
						ctx.statusElement.error('Bộ lọc chỉnh sửa không cho phép sửa đổi: "' + desc + '".');
					} else if (errorCode === 'abusefilter-warning') {
						ctx.statusElement.error([ 'Một cảnh báo đã được phản hồi từ bộ lọc chỉnh sửa: "', desc, '". Nếu bạn muốn tiếp tục chỉnh sửa, vui lòng thực hiện lại. Cảnh báo này sẽ không xuất hiện lần thứ hai.' ]);
						...
						ctx.statusElement.error('Bộ lọc không cho phép chỉnh sửa.');
					}
				// check for blacklist hits
					...
					ctx.statusElement.error('Không thể lưu trang vì URL' + spam + ' nằm trong danh sách đen về spam (spamblacklist)');
				} else {
					ctx.statusElement.error('Không lưu được sửa đổi: ' + ctx.saveApi.getErrorText());
				}
				ctx.editMode = 'all';  // cancel append/prepend/revert modes
		...
		var fnLookupCreationSuccess = function() {
			...
			if (!ctx.lookupNonRedirectCreator || !/^\s*#redirect/i.test($(xml).find('rev').text())) {
				...
				if (!ctx.creator) {
					ctx.statusElement.error('Không thể tìm thấy tên của người tạo trang');
					return;
				}
				...
				ctx.lookupCreationApi = new Morebits.wiki.api('Đang lấy thông tin tạo trang', ctx.lookupCreationApi.query, fnLookupNonRedirectCreator, ctx.statusElement);
				ctx.lookupCreationApi.setParent(this);
				ctx.lookupCreationApi.post();
			}
		...
		var fnLookupNonRedirectCreator = function() {
			...
			if (!ctx.timestamp) {
				ctx.statusElement.error('Không thể tìm thấy nhãn thời gian của quá trình tạo trang');
				return;
			}
		...
		var fnProcessMove = function() {
			...
			if (fnCanUseMwUserToken('move')) {
				...
				if ($(xml).find('page').attr('missing') === '') {
					ctx.statusElement.error('Không thể di chuyển trang bởi vì nó không tồn tại');
					ctx.onMoveFailure(this);
					return;
				}
				...
				if (Morebits.userIsSysop) {
					...
					if (editprot.length > 0 && editprot.attr('level') === 'sysop' && !ctx.suppressProtectWarning &&
						!confirm('Bạn sắp di chuyển trang được khóa hoàn toàn "' + ctx.pageName +
						(editprot.attr('expiry') === 'infinity' ? '" (khóa vô hạn)' : '" (thời hạn khóa ' + new Morebits.date(editprot.attr('expiry')).calendar('utc') + ' (UTC))') +
						'.  \n\nNhấp vào OK để tiếp tục di chuyển hoặc Hủy bỏ để bỏ qua thao tác di chuyển này.')) {
						ctx.statusElement.error('Di chuyển trang được khóa hoàn toàn đã bị hủy bỏ.');
						ctx.onMoveFailure(this);
						return;
					}
				}
				...
				if (!token) {
					ctx.statusElement.error('Không truy xuất được mã token di chuyển.');
					ctx.onMoveFailure(this);
					return;
				}
			...
			ctx.moveProcessApi = new Morebits.wiki.api('đang di chuyển trang...', query, ctx.onMoveSuccess, ctx.statusElement, ctx.onMoveFailure);
		...
		var fnProcessPatrol = function() {
			...
			var patrolStat = new Morebits.status('Đánh dấu trang là đã tuần tra');
	
			ctx.patrolProcessApi = new Morebits.wiki.api('đang tuần tra trang...', query, null, patrolStat);
		...
		var fnProcessTriage = function() {
			...
			var triageStat = new Morebits.status('Đánh dấu trang là đã được quản lý');
	
			ctx.triageProcessApi = new Morebits.wiki.api('đang quản lý trang...', query, null, triageStat, fnProcessTriageError);
		...
		var fnProcessDelete = function() {
			...
			if (fnCanUseMwUserToken('delete')) {
				...
				if ($(xml).find('page').attr('missing') === '') {
					ctx.statusElement.error('Không thể xóa trang vì nó không còn tồn tại');
					ctx.onDeleteFailure(this);
					return;
				}
				...
				var editprot = $(xml).find('pr[type="edit"]');
				if (editprot.length > 0 && editprot.attr('level') === 'sysop' && !ctx.suppressProtectWarning &&
					!confirm('Bạn sắp khôi phục tạo trang được khóa hoàn toàn "' + ctx.pageName +
					(editprot.attr('expiry') === 'infinity' ? '" (khóa vô hạn)' : '" (thời hạn khóa ' + new Morebits.date(editprot.attr('expiry')).calendar('utc') + ' (UTC))') +
					'.  \n\nNhấp vào OK để tiếp tục xóa hoặc Hủy để bỏ qua thao tác xóa này.')) {
					ctx.statusElement.error('Việc xóa trang được khóa hoàn toàn đã bị hủy bỏ.');
					ctx.onDeleteFailure(this);
					return;
				}
				...
				token = $(xml).find('tokens').attr('csrftoken');
				if (!token) {
					ctx.statusElement.error('Không truy xuất được token thông báo xóa.');
					ctx.onDeleteFailure(this);
					return;
				}
			...
			ctx.deleteProcessApi = new Morebits.wiki.api('đang xóa trang...', query, ctx.onDeleteSuccess, ctx.statusElement, fnProcessDeleteError);
		...
		var fnProcessDeleteError = function() {
			...
			if (errorCode === 'internal_api_error_DBQueryError' && ctx.retries++ < ctx.maxRetries) {
				ctx.statusElement.info('Lỗi truy vấn cơ sở dữ liệu, đang thử lại');
				--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
				ctx.deleteProcessApi.post(); // give it another go!
	
			} else if (errorCode === 'missingtitle') {
				ctx.statusElement.error('Không thể xóa trang bởi vì nó không tồn tại');
				if (ctx.onDeleteFailure) {
					ctx.onDeleteFailure.call(this, ctx.deleteProcessApi);  // invoke callback
				}
			// hard error, give up
			} else {
				ctx.statusElement.error('Không xóa được trang: ' + ctx.deleteProcessApi.getErrorText());
				if (ctx.onDeleteFailure) {
					ctx.onDeleteFailure.call(this, ctx.deleteProcessApi);  // invoke callback
				}
			}
		...
		var fnProcessUndelete = function() {
			...
			if (fnCanUseMwUserToken('undelete')) {
				...
				if ($(xml).find('page').attr('missing') !== '') {
					ctx.statusElement.error('Không thể phục hồi trang vì nó đã tồn tại');
					ctx.onUndeleteFailure(this);
					return;
				}
				...
				if (editprot.length > 0 && editprot.attr('level') === 'sysop' && !ctx.suppressProtectWarning &&
					!confirm('Bạn sắp khôi phục tạo trang được khóa hoàn toàn "' + ctx.pageName +
					(editprot.attr('expiry') === 'infinity' ? '" (khóa vô hạn)' : '" (thời hạn khóa ' + new Morebits.date(editprot.attr('expiry')).calendar('utc') + ' (UTC))') +
					'.  \n\nNhấp vào OK để tiếp tục khôi phục hoặc Hủy bỏ để bỏ qua quá trình khôi phục này.')) {
					ctx.statusElement.error('Đã hủy bỏ quá trình phục hồi trang được khóa hoàn toàn.');
					ctx.onUndeleteFailure(this);
					return;
				}
				...
				token = $(xml).find('tokens').attr('csrftoken');
				if (!token) {
					ctx.statusElement.error('Không truy xuất được token thông báo phục hồi.');
					ctx.onUndeleteFailure(this);
					return;
				}
			...
			ctx.undeleteProcessApi = new Morebits.wiki.api('đang phục hồi trang...', query, ctx.onUndeleteSuccess, ctx.statusElement, fnProcessUndeleteError);
		...
		var fnProcessUndeleteError = function() {
			...
			if (errorCode === 'internal_api_error_DBQueryError') {
				if (ctx.retries++ < ctx.maxRetries) {
					...
					ctx.statusElement.error('Lỗi truy vấn cơ sở dữ liệu lặp lại, vui lòng thử lại');
					if (ctx.onUndeleteFailure) {
						ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi);  // invoke callback
					}
				}
			} else if (errorCode === 'cantundelete') {
				ctx.statusElement.error('Không thể phục hồi trang, vì không có bản sửa đổi nào để phục hồi hoặc vì nó đã được phục hồi');
				if (ctx.onUndeleteFailure) {
					ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi);  // invoke callback
				}
			// hard error, give up
			} else {
				ctx.statusElement.error('Không thể phục hồi trang: ' + ctx.undeleteProcessApi.getErrorText());
				if (ctx.onUndeleteFailure) {
					ctx.onUndeleteFailure.call(this, ctx.undeleteProcessApi);  // invoke callback
				}
			}
		...
		var fnProcessProtect = function() {
			...
			if ((ctx.protectEdit || ctx.protectMove) && missing) {
				ctx.statusElement.error('Không thể khóa trang bởi vì nó không tồn tại');
				ctx.onProtectFailure(this);
				return;
			}
			if (ctx.protectCreate && !missing) {
				ctx.statusElement.error('Không thể tạo khóa trang bởi vì nó không tồn tại');
				ctx.onProtectFailure(this);
				return;
			}
			...
			if (!token) {
				ctx.statusElement.error('Không truy xuất được mã token.');
				ctx.onProtectFailure(this);
				return;
			}
			...
			ctx.protectProcessApi = new Morebits.wiki.api('đang bảo vệ trang...', query, ctx.onProtectSuccess, ctx.statusElement, ctx.onProtectFailure);
	...
	*/
	...
	Morebits.wiki.preview = function(previewbox) {
		...
		this.beginRender = function(wikitext, pageTitle) {
			...
			var renderApi = new Morebits.wiki.api('đang tải...', query, fnRenderSuccess, new Morebits.status('Preview'));
			renderApi.post();
		};
	...
	Morebits.wikitext.page.prototype = {
		...
		removeLink: function(link_target) {
			...
			var special_ns_re = /^(?:[Ff]ile|[Ii]mage|[Tt]ập( |_)+[Tt]in|[Hh]ình|[Cc]ategory):/;
		...
		commentOutImage: function(image, reason) {
			...
			var links_re = new RegExp('\\[\\[(?:[Ii]mage|[Ff]ile|[Tt]ập( |_)+[Tt]in|[Hh]ình):\\s*' + image_re_string);
			...
			var gallery_image_re = new RegExp('(^\\s*(?:[Ii]mage|[Ff]ile|[Tt]ập( |_)+[Tt]in|[Hh]ình):\\s*' + image_re_string + '.*?$)', 'mg');
			...
			var free_image_re = new RegExp('(\\|\\s*(?:[\\w\\s]+\\=)?\\s*(?:(?:[Ii]mage|[Ff]ile|[Tt]ập( |_)+[Tt]in|[Hh]ình):\\s*)?' + image_re_string + ')', 'mg');
		...
		addToImageComment: function(image, data) {
			...
			var image_re_string = '(?:[Ii]mage|[Ff]ile|[Tt]ập( |_)+[Tt]in|[Hh]ình):\\s*' + first_char_regex + Morebits.string.escapeRegExp(image.substr(1));
	...
	Morebits.batchOperation = function(currentAction) {
		var ctx = {
			...
			statusElement: new Morebits.status(currentAction || 'Thực hiện hành động hàng loạt'),
		...
		this.run = function(worker, postFinish) {
			if (ctx.running) {
				ctx.statusElement.error('Hoạt động hàng loạt đã đang chạy');
				return;
			}
			ctx.running = true;
			...
			if (arg instanceof Morebits.wiki.api || arg instanceof Morebits.wiki.page) {
				...
				if (ctx.options.preserveIndividualStatusLines) {
					if (arg.getPageName || arg.pageName || (arg.query && arg.query.title)) {
						...
						statelem.info(['đã hoàn tất (', createPageLink(pageName), ')']);
					} else {
						// we don't know the page title - just display a generic message
						statelem.info('đã xong');
					}
				} else {
				...
				new Morebits.status(arg, ['đã xong (', createPageLink(arg), ')']);
			}
		...
		var fnDoneOne = function() {
			...
			if (ctx.countFinished < total) {
				...
				var statusString = 'Đã xong (' + ctx.countFinishedSuccess +
					'/' + ctx.countFinished + ' tác vụ đã thực hiện thành công)';
				...
				ctx.statusElement.warn('Đã xong (vượt qua bởi ' + (ctx.countFinished - total) + ')');
				Morebits.wiki.removeCheckpoint();
				ctx.running = false;
			}
		};
	...
	Morebits.simpleWindow.prototype = {
		...
		addContent: function(content) {
			...
			$(this.content).find('input[type="submit"], button[type="submit"]').each(function(key, value) {
				...
				button.textContent = value.hasAttribute('value') ? value.getAttribute('value') : value.textContent ? value.textContent : 'Thực hiện thao tác';
